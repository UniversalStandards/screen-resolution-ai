...
class DisplayManager:
    @staticmethod
    def get_current_settings(device=None):
        dm = DEVMODEW()
    ...
    @staticmethod
    def enumerate_modes(device=None):
        modes, dm, i = [], DEVMODEW(), 0
        dm.dmSize = ctypes.sizeof(DEVMODEW)
        dev_arg = device or ctypes.c_wchar_p(None)
    ...
    @staticmethod
    def set_resolution(width, height, frequency=0, device=None):
        dm = DEVMODEW()
        dm.dmSize = ctypes.sizeof(DEVMODEW)
        dev_arg = device or ctypes.c_wchar_p(None)
        ctypes.windll.user32.EnumDisplaySettingsW(dev_arg, ENUM_CURRENT_SETTINGS, ctypes.byref(dm))
        ...
        if frequency:
            dm.dmDisplayFrequency = frequency; dm.dmFields |= DM_DISPLAYFREQUENCY
    ...
    def set_dpi_scaling(dpi):
        try:
            ...
            return True, f"DPI set to {dpi} ({round(dpi/96*100)}%). Sign out and back in to apply."
        except Exception as e:
    ...
    def set_refresh_rate(hz, device=None):
        ...
        return False, f"Failed to set {hz}Hz - may not be supported."
    ...
    def get_monitors():
        ...
        def callback(hMonitor, hdcMonitor, lprcMonitor, dwData):
            ...
            monitors.append({"handle": hMonitor,
                "left": struct.unpack_from("i", info, 4)[0], "top": struct.unpack_from("i", info, 8)[0],
                "right": struct.unpack_from("i", info, 12)[0], "bottom": struct.unpack_from("i", info, 16)[0],
    ...
    def restart_graphics_driver():
        VK_B, VK_WIN, VK_CTRL, VK_SHIFT, KEYEVENTF_KEYUP = 0x42, 0x5B, 0x11, 0x10, 0x0002
        ki = ctypes.windll.user32.keybd_event
        for k in [VK_WIN, VK_CTRL, VK_SHIFT, VK_B]: ki(k, 0, 0, 0)
        time.sleep(0.05)
        for k in reversed([VK_WIN, VK_CTRL, VK_SHIFT, VK_B]): ki(k, 0, KEYEVENTF_KEYUP, 0)
    ...
    def get_full_system_snapshot():
        ...
        ok, gpu_info = DisplayManager.run_powershell(
            "Get-WmiObject Win32_VideoController | Select-Object Name,CurrentHorizontalResolution,"
            "CurrentVerticalResolution,CurrentRefreshRate,CurrentBitsPerPixel,DriverVersion,VideoModeDescription"
...
SYSTEM_PROMPT = """You are the Screen Resolution AI Assistant - an expert Windows display engineer.

Available tools (executed by the host app):
  set_resolution(width, height, frequency?), set_refresh_rate(hz), set_dpi(dpi_value),
  restart_gpu_driver(), clear_font_cache(), toggle_cleartype(enable), run_powershell(command),
  open_settings(panel)

Always return valid JSON only, with this schema:
{
  "diagnosis": "Short description",
  "explanation": "Explanation in plain English",
  "actions": [{"id": 1, "label": "...", "tool": "...", "params": {}, "risk": "low|medium|high", "reversible": true, "reason": "..."}],
  "warnings": [],
  "requires_restart": false,
  "follow_up": "What to check after"
}
No markdown fences. No preamble. Only JSON."""
...
class AIEngine:
    ...
    def _call(self, messages):
        response = self.client.messages.create(model=MODEL, max_tokens=2048,
        ...
        try:
            ...
            return {"diagnosis": "Parse error", "explanation": raw, "actions": [],
                    "warnings": ["Could not parse response."], "requires_restart": False, "follow_up": ""}

    def analyze(self, user_message, snapshot):
        content = f"SYSTEM SNAPSHOT:\n```json\n{json.dumps(snapshot, indent=2)}\n```\n\nUSER PROBLEM:\n{user_message}"
        self.history.append({"role": "user", "content": content})
        result = self._call(self.history)
        self.history.append({"role": "assistant", "content": json.dumps(result)})
        return result

    def followup(self, user_message):
...
class ActionExecutor:
    ...
    def execute(self, action):
        ...
        try:
            ...
            elif tool == "set_refresh_rate": return self.dm.set_refresh_rate(int(params["hz"]))
            elif tool == "set_dpi": return self.dm.set_dpi_scaling(int(params["dpi_value"]))
            elif tool == "restart_gpu_driver": return self.dm.restart_graphics_driver()
            elif tool == "clear_font_cache": return self.dm.clear_font_cache()
            elif tool == "toggle_cleartype": return self.dm.toggle_cleartype(bool(params.get("enable", True)))
            elif tool == "run_powershell": return self.dm.run_powershell(params.get("command", ""))
            elif tool == "open_settings":
                os.startfile(params.get("panel", "ms-settings:display"))
                return True, f"Opened: {params.get('panel')}"
            return False, f"Unknown tool: {tool}"
        except Exception as e:
            log.error(f"Execution error: {e}"); return False, str(e)

class APIKeyDialog(ctk.CTkToplevel):
    def __init__(self, parent, current_key=""):
        super().__init__(parent)
        self.title("Configure API Key"); self.geometry("480x260")
        self.resizable(False, False); self.grab_set()
        self.result = None; self.configure(fg_color=BG_MID)
        ctk.CTkLabel(self, text="API Key - Anthropic", font=("Consolas", 16, "bold"), text_color=FG_MAIN).pack(pady=(24, 4))
        ...
        if current_key: self.key_entry.insert(0, current_key)
        self.show_var = tk.BooleanVar()
        ctk.CTkCheckBox(self, text="Show key", variable=self.show_var, font=("Consolas", 11),
            text_color=FG_DIM, command=self._toggle_show).pack(pady=(4, 16))
        btns = ctk.CTkFrame(self, fg_color="transparent"); btns.pack()
        ctk.CTkButton(btns, text="Cancel", width=100, fg_color=BG_LIGHT, hover_color=BG_MID,
            command=self.destroy).pack(side="left", padx=8)
        ctk.CTkButton(btns, text="Save", width=120, fg_color=ACCENT, hover_color=ACCENT2,
    ...
    def _save(self):
        ...
        if not key.startswith("sk-ant-"):
            messagebox.showwarning("Invalid Key", "Key should start with 'sk-ant-'.", parent=self); return
        self.result = key; self.destroy()

class ActionCard(ctk.CTkFrame):
    RISK_COLORS = {"low": SUCCESS, "medium": WARN, "high": DANGER}
    def __init__(self, parent, action, index, on_execute):
        ...
        risk = action.get("risk", "low"); color = self.RISK_COLORS.get(risk, SUCCESS)
        top = ctk.CTkFrame(self, fg_color="transparent"); top.pack(fill="x", padx=12, pady=(10, 2))
        ctk.CTkLabel(top, text=f"  {index}  ", font=("Consolas", 11, "bold"), fg_color=color,
            text_color="white", corner_radius=4, width=28).pack(side="left")
        ...
        if action.get("reason"):
            ctk.CTkLabel(self, text=action["reason"], font=("Consolas", 10), text_color=FG_DIM,
                anchor="w", wraplength=540, justify="left").pack(fill="x", padx=12, pady=(0, 4))
        ctk.CTkLabel(self, text=f"Tool: {action.get('tool','')}  |  Reversible: {'Y' if action.get('reversible') else 'N'}",
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1100x740"); self.minsize(900, 640); self.configure(fg_color=BG_DARK)
        try: self.iconbitmap(default="icon.ico")
        except: pass
        self.cfg = load_config(); self.dm = DisplayManager()
        self.executor = ActionExecutor(self.dm); self.engine = None; self.snapshot = None
        self._init_engine(); self._build_ui(); self._refresh_snapshot()

    def _init_engine(self):
        key = self.cfg.get("api_key", "")
        if key:
            try: self.engine = AIEngine(key); log.info("AI engine initialized.")
            except Exception as e: log.error(f"Engine init failed: {e}"); self.engine = None

    def _build_ui(self):
        bar = ctk.CTkFrame(self, fg_color=BG_MID, height=52, corner_radius=0); bar.pack(fill="x")
        bar.pack_propagate(False)
        ctk.CTkLabel(bar, text="Screen Resolution AI Assistant", font=("Consolas", 15, "bold"),
            text_color=FG_MAIN, anchor="w").pack(side="left", padx=20)
        for text, cmd in [("API Key", self._open_api_dialog), ("Refresh", self._refresh_snapshot),
                          ("Reset Session", self._reset_session)]:
            ctk.CTkButton(bar, text=text, width=110, height=32, font=("Consolas", 11),
                fg_color=BG_LIGHT, hover_color=BG_MID, command=cmd).pack(side="right", padx=4)
        body = ctk.CTkFrame(self, fg_color="transparent"); body.pack(fill="both", expand=True, padx=12, pady=8)
    ...
    def _build_left_panel(self, parent):
        ...
        self.modes_box.pack(fill="x", padx=8, pady=(0, 12)); self.modes_box.configure(state="disabled")
    ...
    def _build_right_panel(self, parent):
        ...
        ctk.CTkLabel(parent, text="SUGGESTED ACTIONS", font=("Consolas", 11, "bold"),
            text_color=ACCENT, anchor="w").pack(fill="x")
        self.actions_frame = ctk.CTkScrollableFrame(parent, fg_color=BG_MID, corner_radius=8,
            height=200, label_text="")
        self.actions_frame.pack(fill="x", pady=(4, 8))
        input_row = ctk.CTkFrame(parent, fg_color="transparent"); input_row.pack(fill="x")
        ...
        self.send_btn = ctk.CTkButton(input_row, text="Analyze", width=120, height=40,
            font=("Consolas", 12, "bold"), fg_color=ACCENT, hover_color=ACCENT2,
    ...
    def _refresh_snapshot(self):
        ...
        def _run():
            try:
                snap = self.dm.get_full_system_snapshot(); self.snapshot = snap
                self.after(0, lambda: self._update_info_panel(snap))
            except Exception as e:
                log.error(f"Snapshot error: {e}"); self.after(0, lambda: self._set_status(f"Snapshot error: {e}"))
        threading.Thread(target=_run, daemon=True).start()

    def _update_info_panel(self, snap):
        pm = snap.get("primary_monitor", {}); dpi = snap.get("dpi_info", {})
        lines = [f"OS:         {snap.get('os', 'N/A')}", f"Monitors:   {snap.get('monitor_count', '?')}",
                 f"Resolution: {pm.get('width', '?')}x{pm.get('height', '?')}",
                 f"Refresh:    {pm.get('frequency', '?')} Hz", f"Bit Depth:  {pm.get('bit_depth', '?')} bpp",
                 f"DPI:        {dpi.get('system_dpi', '?')}", f"Scale:      {dpi.get('scale_percent', '?')}%",
        ...
        self._set_textbox(self.modes_box, "\n".join([f"{m['width']}x{m['height']} @ {m['frequency']}Hz" for m in modes[:25]]) or "No modes found.")
        self._set_status("System snapshot updated.")

    def _send_message(self):
        msg = self.msg_entry.get().strip()
        if not msg: return
        if not self.engine: self._open_api_dialog(); return
        ...
        def _run():
            try:
                if not self.snapshot: self.snapshot = self.dm.get_full_system_snapshot()
                result = self.engine.analyze(msg, self.snapshot) if not self.engine.history else self.engine.followup(msg)
                self.after(0, lambda: self._handle_ai_response(result))
            except anthropic.AuthenticationError: self.after(0, self._on_auth_error)
    ...
    def _handle_ai_response(self, result):
        ...
        parts = []
        if result.get("diagnosis"): parts.append(f"DIAGNOSIS\n{result['diagnosis']}")
        if result.get("explanation"): parts.append(f"EXPLANATION\n{result['explanation']}")
        if result.get("warnings"): parts.append("WARNINGS\n" + "\n".join(f"  * {w}" for w in result["warnings"]))
        if result.get("follow_up"): parts.append(f"FOLLOW-UP\n{result['follow_up']}")
        ...
        if actions:
            for i, act in enumerate(actions, 1): ActionCard(self.actions_frame, act, i, self._confirm_and_execute)
        else:
            ctk.CTkLabel(self.actions_frame, text="No actions recommended. Clarify your issue.",
    ...
    def _confirm_and_execute(self, action):
        ...
        msg = (f"Execute: {action.get('label')}\n\nRisk: {action.get('risk','low').upper()}\n{rev}\n\n{action.get('reason','')}\n\nProceed?")
        if not messagebox.askyesno("Confirm Action", msg): return
        self._set_status(f"Executing: {action.get('label')}...")
        def _run():
            ok, output = self.executor.execute(action)
            self.after(0, lambda: self._on_exec_result(ok, output, action.get("label", "")))
        threading.Thread(target=_run, daemon=True).start()

    def _on_exec_result(self, ok, output, label):
        self._add_chat_bubble("System", f"{'OK' if ok else 'FAIL'}: {label}\n{output}",
            is_user=False, color=SUCCESS if ok else DANGER)
        self._set_status(f"{'Success' if ok else 'Failed'}: {label}")
        if ok: self._refresh_snapshot()

    def _add_chat_bubble(self, sender, text, is_user, color=None):
        ...
        hdr = ctk.CTkFrame(bubble, fg_color="transparent"); hdr.pack(fill="x", padx=10, pady=(8, 2))
        ctk.CTkLabel(hdr, text=sender, font=("Consolas", 10, "bold"),
            text_color=color or (ACCENT if is_user else FG_DIM)).pack(side="left")
        ctk.CTkLabel(hdr, text=datetime.now().strftime("%H:%M:%S"), font=("Consolas", 9),
            text_color=BG_LIGHT).pack(side="right")
    ...
    def _open_api_dialog(self):
        dlg = APIKeyDialog(self, self.cfg.get("api_key", "")); self.wait_window(dlg)
        if dlg.result:
            self.cfg["api_key"] = dlg.result; save_config(self.cfg); self._init_engine()
    ...
    def _on_auth_error(self):
        self.send_btn.configure(state="normal", text="Analyze")
        messagebox.showerror("Auth Error", "Invalid API key. Update via API Key button.")
